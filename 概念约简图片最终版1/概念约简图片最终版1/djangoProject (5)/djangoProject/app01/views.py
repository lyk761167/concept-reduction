        ...
        try:
            ...
            for row in matrix_str.split(']'):
                if row:
                    ...
                    matrix.append(int_row[:6])

            print(f"Parsed matrix: {matrix}")

            def number_to_image(num):
                mapping = {1: 's.jpg', 2: 't.jpg', 3: 'c.jpg', 4: 'd.jpg', 5: 'e.jpg', 6: 'f.jpg'}
                return mapping.get(num, f"{num}.jpg")

            result = dynamic_add_3(matrix)
            converted_result = [{number_to_image(a), number_to_image(b)} for a, b in result]
            print(f"Result: {result}")
            print(f"Converted Result: {converted_result}")

            # 将 set 集合转换为列表
            result_list = [list(row) for row in converted_result]
